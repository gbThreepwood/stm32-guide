 ...
 *
 * I would prefer a library which was not tied to the GPIO
 * functions of my specific MCU. E.g. something which required
 * the user to implement some callback functions, but I could not
 * find anything in a quick google search, and I am not willing to
 * prioritize implementing it myself atm.
 *
...
void
HD44780_Init(uint8_t cols, uint8_t rows) {
    /* Initialize delay */
    TM_DELAY_Init();
...
void
HD44780_Puts(uint8_t x, uint8_t y, char* str) {
...
void
HD44780_PutCustom(uint8_t x, uint8_t y, uint8_t location) {
    TM_HD44780_CursorSet(x, y);
